...
export function useVideoPlayer(): UseVideoPlayerReturn {
  ...
  function handleTimeUpdate() {
    ...
    rafId = requestAnimationFrame(() => {
      ...
      if (time >= currentSegment.endTime) {
        ...
        // 檢查使用者是否手動拖動到非片段區域
        const isInAnySegment = segments.some((seg) => time >= seg.startTime && time < seg.endTime);
   ...
   */
  function findNearestSegment(time: number, segmentList: TimeSegment[]): TimeSegment | null {
