...
player.on('timeupdate', () => {
  ...
  // 檢查使用者是否手動拖動到非片段區域
  const isInAnySegment = segments.value.some(
    (seg) => currentTime >= seg.start.seconds && currentTime < seg.end.seconds
  );
...
container.register(
  'UploadVideoUseCase',
  () => new UploadVideoUseCase(container.get('VideoRepository'), container.get('FileStorage'))
);
```
